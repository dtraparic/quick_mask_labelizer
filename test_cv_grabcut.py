...
if __name__ == '__main__':
    ...
    filename = './static/images_edit/FudanPed00001.png'
    ...
    while(1):
        ...
        if k == 27:         # esc to exit
            ...
            if (rect_or_mask == 0):         # grabcut with rect
                ...
                # print("img", type(img2), img2.shape, img2.dtype)
                # print("mask", type(mask), mask.shape, mask.dtype, np.unique(mask))
                # print("rect", type(rect), rect)
                cv2.grabCut(img2, mask, rect, bgdmodel, fgdmodel, 5, cv2.GC_INIT_WITH_RECT)
                print("maskAFTER", type(mask), mask.shape, mask.dtype, np.unique(mask))
                rect_or_mask = 1
